...
async def on_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── SESSION CHECK ────────────────────────────────────────
    # FIX: new users (never had a session) get auto-session instead of locked screen
    if not session_alive(uid):
        pin_hash = db_get_pin(uid)
        if pin_hash:
            # Has a PIN set → they must unlock
            await send_locked(update)
        else:
            # New user or session expired with no PIN → auto-create session
            session_touch(uid)
            await update.message.reply_text(
                home_text(uid, update.effective_user.first_name),
                parse_mode=ParseMode.MARKDOWN,
