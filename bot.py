...
# ── Index setup: drop any legacy wrong-field indexes, then ensure correct ones ──
def _setup_indexes() -> None:
    # Drop the legacy "user_id_1" index if it exists — this was the root cause
    # of DuplicateKeyError: { user_id: null } for every new user.
...
def db_upsert_user(uid: int, name: str, username: Optional[str]) -> None:
    ...
    try:
        ...
        # Race condition — doc was just inserted by another request, just update
        col_users.update_one(
            {"uid": uid},
            {"$set": {"last_seen": now, "name": name}},
        )
...
def parse_otpauth(uri: str) -> Optional[dict]:
    ...
    if not m:
        return None
    # URL-decode the label (e.g. %40 → @, %20 → space)
    label  = unquote(m.group(1)).strip()
    # URL-decode each param value too
    params = {
        k: unquote(v)
        for k, v in (
    ...
    if ":" in label:
        ...
        account = label
    # Clean up svc: strip whitespace, collapse internal spaces
...
# Global account list cache per user — maps short index → svc name
# so callback_data stays tiny (e.g. "DETAIL:0") and never hits Telegram's 64-byte limit.
_acct_cache: dict[int, list] = {}   # uid → sorted svc list


def _cache_accounts(uid: int, docs: list) -> list:
    """Sort docs, store svc names in cache, return sorted docs."""
...
def svc_from_index(uid: int, idx: int) -> Optional[str]:
    """Resolve a short numeric index back to the service name."""
...
def ikb_accounts(docs: list, prefix: str, page: int = 0,
    ...
    sorted_docs = _cache_accounts(uid, docs) if uid else sorted(docs, key=lambda x: x["svc"].lower())
    ...
    for abs_idx, d in enumerate(sorted_docs[start:end], start=start):
        ...
        if d.get("issuer") and d["issuer"] != d["svc"]:
            label += f"  · {d['issuer']}"
        # callback_data uses numeric index — stays well under 64 bytes
        cb = f"{prefix}:{abs_idx}"
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    def _resolve(raw: str) -> Optional[str]:
        """Convert callback payload to svc name.
        Payload is either a numeric index (new style) or raw svc name (legacy).
        """
        if raw.isdigit():
            return svc_from_index(uid, int(raw))
        return raw  # legacy fallback for old messages still in chat

    # ── OTP GET ─────────────────────────────────────────────
    if data.startswith("OTP_GET:"):
        svc = _resolve(data.split(":", 1)[1])
        if not svc:
            await q.answer("Session expired — please tap 🔑 Get OTP again.", show_alert=True); return
        _cancel_refresh(uid)
        doc = db_get(uid, svc)
        if not doc:
    ...
    # ── DETAIL ──────────────────────────────────────────────
        ...
        svc = _resolve(data.split(":", 1)[1])
        if not svc:
            await q.answer("Session expired — please tap 📋 My Accounts again.", show_alert=True); return
        _cancel_refresh(uid)
        doc = db_get(uid, svc)
        if not doc:
            await q.edit_message_text("❌ Account not found."); return
    ...
    # ── EXPORT URI ──────────────────────────────────────────
        ...
        svc = _resolve(data.split(":", 1)[1]) or data.split(":", 1)[1]
        doc = db_get(uid, svc)
        if not doc:
    ...
    # ── RENAME from inline ───────────────────────────────────
        ...
        svc = _resolve(data.split(":", 1)[1])
        if not svc:
            await q.answer("Session expired — tap ✏️ Rename again.", show_alert=True); return
        _cancel_refresh(uid)
    ...
    # ── DELETE ASK ──────────────────────────────────────────
        ...
        svc = _resolve(data.split(":", 1)[1])
        if not svc:
            await q.answer("Session expired — tap 🗑 Delete Account again.", show_alert=True); return
        _cancel_refresh(uid)
        await q.edit_message_text(
    ...
    # ── UPDATE SECRET (duplicate resolution) ────────────────
        ...
        # callback_data carries the raw svc name — use it directly, not _resolve()
        svc    = data.split(":", 1)[1]
        parsed = ctx.user_data.pop("pending_update", None)
        ctx.user_data.pop("pending_update_svc", None)
        ctx.user_data.clear()
        if not parsed:
...
async def _save_and_show(msg, uid: int, parsed: dict, ctx=None):
    """Encrypt, save, show first OTP. Returns otp_msg on success, None on duplicate."""
    ...
    if not ok:
        if ctx is not None:
            ...
            ctx.user_data["pending_update_svc"] = parsed["svc"]
        await msg.edit_text(
        ...
        return None
    # Edit spinner to success header, then send OTP as a separate message.
    # Never nest otp_text() inside another f-string — its backtick blocks
    # break Telegram's Markdown parser when wrapped in more formatting.
    await msg.edit_text(
...
async def _do_stats(update: Update, uid: int) -> None:
    ...
    if docs:
        dates = [
            d["created"].replace(tzinfo=timezone.utc) if d["created"].tzinfo is None else d["created"]
            for d in docs
        ]
