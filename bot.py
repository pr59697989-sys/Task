...
def parse_otpauth(uri: str) -> Optional[dict]:
    ...
    if ":" in label:
        ...
        account = label
    return {
        "svc":       (account.strip() or issuer.strip())[:64].strip(),
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── UPDATE SECRET (duplicate resolution) ────────────────
        ...
        # callback_data carries the raw svc name (not an index) — use it directly
        svc    = data.split(":", 1)[1]
...
async def _save_and_show(msg, uid: int, parsed: dict, ctx=None):
    ...
    if not ok:
        # Offer update — keep pending data in user_data so session_touch doesn't wipe it
        if ctx is not None:
    ...
    # Show the OTP text directly — never nest otp_text() inside another f-string
    # because it contains backtick blocks that break Telegram's Markdown parser.
    await msg.edit_text(
